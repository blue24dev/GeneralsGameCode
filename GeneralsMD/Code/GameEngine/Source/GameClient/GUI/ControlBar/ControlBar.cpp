...
const FieldParse CommandSet::m_commandSetFieldParseTable[] =
{
	...
	//MODDD - that first row's 3rd collumn alue was 'nullptr' instead of '0' to agree with the rest of the column being numbers?
	// weird automatic change maybe?
	...
	for( i = 0; i < m_currentlyUsedSpecialPowersButtons; i++ )
	{
		...
		else
		{
			...
			if( BitIsSet( commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE ) )
			{
				...
				Object *obj = ThePlayerList->getLocalPlayer()->findMostReadyShortcutSpecialPowerOfType( commandButton->getSpecialPowerTemplate()->getSpecialPowerTypeUnique() );
				if( !obj )
				{
					continue;
				}
...
void ControlBar::drawSpecialPowerShortcutMultiplierText()
{
	for( Int i = 0; i < m_currentlyUsedSpecialPowersButtons; i++ )
	{
		...
		for( int i = 0; i < MAX_SPECIAL_POWER_SHORTCUTS; i++ )
		{
			...
			if( spTemplate )
			{
				numReady = ThePlayerList->getLocalPlayer()->countReadyShortcutSpecialPowersOfType( spTemplate->getSpecialPowerTypeUnique() );
			}
