...
void AI::parseAiDataDefinition( INI* ini )
{
	if( TheAI )
	{
		...
		if (TheAI->m_aiData->m_guardInnerModifierHuman < 2.0) {
			TheAI->m_aiData->m_guardInnerModifierHuman = 2.0;
		}
		if (TheAI->m_aiData->m_guardOuterModifierHuman < 2.3) {
			TheAI->m_aiData->m_guardOuterModifierHuman = 2.3;
		}
