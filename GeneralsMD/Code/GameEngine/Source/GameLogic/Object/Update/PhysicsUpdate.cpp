...
void PhysicsBehavior::onCollide( Object *other, const Coord3D *loc, const Coord3D *normal )
{
	...
	// Also, require this object to be intending to 'enter' something.
	...
	// --- BLEND
	/*
	Bool enterBasedIntent = (ai->getAIStateType() == AI_ENTER || ai->getAIStateType() == AI_HIJACK);
	if (ai != nullptr &&
		(
			ai->getIgnoredObstacleID() == other->getID() ||
			(
				enterBasedIntent && ai->getGoalObject() && ai->getGoalObject()->getID() == other->getID()
			)
		)
	)
	*/
	// --- Final
	Bool neutralEntryCheck = false;
	if (ai != nullptr)
	{
		Bool enterBasedIntent = (ai->getAIStateType() == AI_ENTER || ai->getAIStateType() == AI_HIJACK);
		neutralEntryCheck =
		(
			...
			(
				(ai->getIgnoredObstacleID() == other->getID()) ||
				(ai->getGoalObject() && ai->getGoalObject()->getID() == other->getID())
			)
		);
	}
	if (neutralEntryCheck)
	// ---
	{
