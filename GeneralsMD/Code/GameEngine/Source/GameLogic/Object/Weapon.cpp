...
void HandleWeaponFireQueuedOCL(Weapon* weapon, Object* source) {
	...
	if (weapon->getWeaponFireQueuedOcl() != nullptr) {
		ObjectCreationList::create( weapon->getWeaponFireQueuedOcl(), source, nullptr );
		weapon->setWeaponFireQueuedOcl( nullptr );
	}
}
...
void Weapon::computeBonus(const Object *source, WeaponBonusConditionFlags extraBonusFlags, WeaponBonus& bonus) const
{
	...
	if (source->isKindOf(KINDOF_FS_BASE_DEFENSE)) {
		...
		bonus.setField(WeaponBonus::RANGE, current + 0.17f);
	}
}
...
)
{
	...
	this->setWeaponFireQueuedOcl( nullptr );
		...
		case DAMAGE_DISARM:
		{
			if (sourceObj && victimObj)
			{
				...
				*/
				...
				LandMineInterface* foundLMI = nullptr;
				for (BehaviorModule** bmi = victimObj->getBehaviorModules(); *bmi; ++bmi)
				{
					LandMineInterface* lmi = (*bmi)->getLandMineInterface();
					if (lmi)
				...
				if (foundLMI != nullptr || victimObj->isKindOf( KINDOF_MINE ) || victimObj->isKindOf( KINDOF_BOOBY_TRAP ) || victimObj->isKindOf( KINDOF_DEMOTRAP ) ) {
					isVictimMine = TRUE;
				}
