...
RebuildHoleBehavior::~RebuildHoleBehavior()
{
	...
	if( m_workerID != INVALID_ID )
	{
		...
		{
#if EXTRA_DEBUG_HELP
			g_destroyObjectSource.push_back(22);
#endif
...
#if EXTRA_DEBUG_HELP
		g_destroyObjectSource.push_back(23);
#endif
...
UpdateSleepTime RebuildHoleBehavior::update()
{
	...
	if( reconstructing && !reconstructing->getStatusBits().test( OBJECT_STATUS_UNDER_CONSTRUCTION ) )
	{
		...
		{
#if EXTRA_DEBUG_HELP
			g_destroyObjectSource.push_back(24);
#endif
			TheGameLogic->destroyObject( worker );
		}
...
#if EXTRA_DEBUG_HELP
		g_destroyObjectSource.push_back(25);
#endif
...
void RebuildHoleBehavior::onDie( const DamageInfo *damageInfo )
{
	if( m_workerID != INVALID_ID )
	{
		...
		{
#if EXTRA_DEBUG_HELP
			g_destroyObjectSource.push_back(26);
#endif
			TheGameLogic->destroyObject(worker);
			m_workerID = INVALID_ID;
		}
	...
	// destroy us
#if EXTRA_DEBUG_HELP
	g_destroyObjectSource.push_back(27);
#endif
